...
Stata script used to modify or define the ultra-processed food (UPF) subgroups prior to the main pipeline.

`2_dietscore.R`
Script used to calculate dietary scores based on the baseline nutritional data.

`3_orc.R`
Script used to pgenerate obesity-related Cancers variable.
...
`7_validation.R`
Script used to run corssvalidation.

`8_sensitivity.R`
Script used to perform sensitivity analyses. 

`X_analysis_functions.R`
Contains statistical helper functions sourced by the main, subgroup, and sensitivity analysis scripts.

`X_functions_ct.R`
Contains helper functions used primarily for data cleaning and table Generation.

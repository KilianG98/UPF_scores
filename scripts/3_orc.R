...
df <- readRDS("data/UKB_data_wdiet3.rds")

#get relevant columns
cols_cancer <- sapply(df, function(x) {
	lab <- attr(x, "label")
	if (is.null(lab)) return(FALSE)
	any(grepl("cancer|death|date|centre|menopause|age|sex", as.character(lab), ignore.case = TRUE))
})
cols_cancer[1] <- T

cancer_df <- df[, cols_cancer]
#name cols to work with script
names(cancer_df) <- gsub("ca_dx_dte", "f.40005", names(cancer_df))
names(cancer_df) <- gsub("_", ".", names(cancer_df)) 
names(cancer_df) <- gsub("ca.icd10", "f.40006", names(cancer_df))
names(cancer_df) <- gsub("ca.icd9", "f.40013", names(cancer_df))
names(cancer_df) <- gsub("ca.icd9", "f.40013", names(cancer_df))
names(cancer_df) <- gsub("ca.bhv", "f.40012", names(cancer_df))
names(cancer_df) <- gsub("n.40011", "f.40011", names(cancer_df))

cancer_df <- cancer_df %>%
	rename(f.53.0.0 = recruit.date0.0) %>%
	rename(f.191.0.0 = lost.followup.date) %>%
	rename(f.40000.0.0 = death.dte) %>%
	rename(eid = n.eid)


#script provided by Michael Stein
#-------------------------------------------------------------------------------
...
ca_icd9 <- cancer_df[ , c("eid","f.40013.0.0", "f.40013.1.0", "f.40013.2.0","f.40013.3.0",
																										"f.40013.4.0", "f.40013.5.0", "f.40013.6.0", "f.40013.7.0", "f.40013.8.0", 
																										"f.40013.10.0", "f.40013.11.0", "f.40013.12.0")] #need, "f.40013.13.0","f.40013.9.0",
...
ca_icd10 <- cancer_df[ , c("eid", "f.40006.0.0", "f.40006.1.0", "f.40006.2.0", "f.40006.3.0",
																											...
																											"f.40006.10.0", "f.40006.11.0",  "f.40006.13.0",  "f.40006.15.0",
																											"f.40006.16.0","f.40006.17.0","f.40006.18.0","f.40006.19.0","f.40006.20.0","f.40006.21.0"
)]#"f.40006.12.0","f.40006.14.0" dont exiust
...
ca_bhv <- cancer_df[ , c("eid",  "f.40012.0.0", "f.40012.1.0", "f.40012.2.0", "f.40012.3.0",
																									...
																									"f.40012.16.0","f.40012.17.0","f.40012.18.0","f.40012.19.0","f.40012.20.0","f.40012.21.0")] #have
...
cancer_df <- cancer_df %>%
	rename(f.54.0.0 = assess.cen) %>%
	rename(f.2724.0.0 = menopause.status.0.0)
...
cancer_df <- cancer_df %>%
	rename(f.21022.0.0 = age.recru)
cancer_df$f.31.0.0 <-as.character(as.numeric(cancer_df$Sex))
...
cancer_df$f.34.0.0 <- as.numeric(substr(as.character(df$recruit_date0_0), 1, 4)) - as.numeric(df$age_recru)
cancer_df$f.21003.0.0 <- df$age_recru
cancer_df$f.40007.0.0 <- cancer_df$n.40007.0.0
...
whicheverfirst <- dates %>%
	pivot_longer(cols = c(date_diagnose, death_date, lost_to_fu_date, end_fu),
														names_to = "whicheverfirst",
														values_to = "date_exit_first")
...
source("scripts/X_functions_ct.r")

#get obesity related cancers (Nazli project)
cancer_data <- get_cancer_vars(cancer_data)

sum(cancer_data$obesity_cancer)
df <- df %>%
	rename(eid = n_eid)

#merge back
df_merged <- merge(df, cancer_data, by = "eid")


# saveRDS(df_merged, "data/UKB_data_all2.rds")

#16.2.26 new version, based on alternative definition of animalp subgroup
saveRDS(df_merged, "data/UKB_data_all3.rds")
...
# 
# 
# sum(df$first_ca_10_dte != df$ca_dx_dte_0_0)
# sum(df$first_ca_10_dte != df$ca_dx_dte_0_0, na.rm = TRUE)
# table(df$first_ca_10_dte != df$ca_dx_dte_0_0, useNA = "ifany")
# 
# df$breast_inc_pm <-
# 	ifelse(df$menopause_status_0_0 == 1, df$breast_inc, 0)
# 
# df$ORC <- 0
# df$ORC <- rowSums(
# 	df[, c(
...
# 
# patterns <- c(
# 	"C900|C901|C902|C903", "C739", "C160", "C180|C181|C182|C183|C184|C185|C186|C187|C188|C189",
# 	"C19|C199", "C209", "C210|C211|C212|C213|C214|C215|C216|C217|C218|C219", "C220|C221|C222|C223|C224|C225|C226|C227|C228|C229",
# 	"C230|C231|C232|C233|C234|C235|C236|C237|C238|C239", "C240|C241|C242|C243|C244|C245|C246|C247|C248|C249", "C250|C251|C252|C253|C254|C255|C256|C257|C258|C259",
# 	"C700|C701|C702|C703|C704|C705|C706|C707|C708|C709","C710|C711|C712|C713|C714|C715|C716|C717|C718|C719","C720|C721|C722|C723|C724|C725|C726|C727|C728|C729", "C751|C752|C753","C540|C541|C542|C543|C544|C545|C546|C547|C548|C549","C550|C551|C552|C553|C554|C555|C556|C557|C558|C559",
# 	"C730|C731|C732|C733|C734|C735|C736|C737|C738|C739", "C569", "C649"
# )
# values <- rep(1, length(patterns))
# 
# for (i in seq_along(patterns)) {
# 	df$ORC[grepl(patterns[i], df$ca_icd10_0_0)] <- values[i] }
...
get_cat_vars <- function (df){
	df <- df %>%
		mutate(
			...
			bmi_bil_cat4 = factor(
				case_when(
					bmi_m >= 16 & bmi_m < 25 & tbili <= 10 ~ 1,
					bmi_m >= 16 & bmi_m < 25 & tbili >= 10 ~ 2,
					bmi_m >= 25 & tbili <= 10 ~ 3,
					bmi_m >= 25 & tbili > 10 ~ 4,
					TRUE ~ 9
				),
				levels = c(1, 2, 3,4, 9),
				labels = c("Normal weight non-GS", "Normal weight GS",
															"Overweight non-GS", "Overweight GS", "Underweight")
			),
			bmi_bil_cat_obese4 = factor(
				case_when(
					bmi_m >= 16 & bmi_m < 25 & tbili <= 10 ~ 1,
					bmi_m >= 16 & bmi_m < 25 & tbili >= 10 ~ 2,
					bmi_m >= 30 & tbili <= 10 ~ 3,
				...
				levels = c(1, 2, 3,4, 9),
				labels = c("Normal weight non-GS", "Normal weight GS",
															"Obese non-GS", "Obese GS", "overweight")
			)
		)
	return(df)
}
...
get_labels <- function(df){
  ...
  label_map <- list(
    ...
    pa_min_per_week_MET_quint = "Quintiles of MET score",
...
generate_table <- function(data, sex, stratify_var, row_vars,cancer_vars) {
  ...
  rndr <- function(x, ...) {
  	if (is.logical(x)) {
  		y <- render.default(x, ...)
  		# Round percentages normally and remove the "%" sign
  		y <- str_replace_all(y, "([0-9]+\\.?[0-9]*)%", function(m) {
  			num <- as.numeric(sub("%", "", m))
  			as.character(round(num))
  		})
  		y[2]
  	} else if (is.numeric(x)) {
  		mean_x <- mean(x, na.rm = TRUE)
  		sd_x <- sd(x, na.rm = TRUE)
  		sprintf("%.1f (%.1f)", mean_x, sd_x)
  	} else {
  		y <- render.default(x, ...)
  		y <- str_replace_all(y, "([0-9]+\\.?[0-9]*)%", function(m) {
  			num <- as.numeric(sub("%", "", m))
  ...
  rndr.cancer <- function(x, ...) {
  	if (is.logical(x)) {
  		y <- render.default(x, ...)
  		# Round percentages normally and remove the "%" sign
  		y <- as.numeric(str_replace_all(y, "\\s*\\([^)]*\\)", ""))
  		y[2]
  	} else if (is.numeric(x)) {
  		mean_x <- mean(x, na.rm = TRUE)
  		sd_x <- sd(x, na.rm = TRUE)
  ...
  create_tablec <- function(formula, data, caption) {
  	table1(formula, data = data, caption = caption, render = rndr.cancer, overall=NULL)#render = rndr.cancer
  }
  ...
  cancer_df <- as.data.frame(table_cancer)
  cancer_df <- cancer_df[-1,]
  print(cancer_df)
  
  cancer_df[, 2:4] <- lapply(cancer_df[, 2:4], function(x) as.numeric(as.character(x)))
  cancer_df_pct <- cancer_df
	
  print(cancer_df[, 2:4] )
  # Loop over the 3 numeric columns
  for (col in 2:4) { 
  	pct <- round(cancer_df[, col] / rowSums(cancer_df[, 2:4]) * 100)
  	cancer_df_pct[, col] <- sprintf("%d (%d%%)", cancer_df[, col], pct) 
  	}
  
  
 	print(cancer_df_pct)
  return(rbind(as.data.frame(table_base), cancer_df_pct))
}
